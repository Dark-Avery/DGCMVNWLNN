...
def louvain_communities(
    ...
    edge_clusters = {edge: {edge} for edge in G.edges()}  # Начальная инициализация каждого ребра как отдельного кластера
...
def louvain_partitions(
    ...
    while improvement:
        ...
        graph = _gen_graph(graph, inner_partition, edge_clusters)  # Обновление графа с учётом новых кластеров рёбер
        update_metrics(graph)  # Пересчёт метрик, если необходимо
...
def _gen_graph(G, partition, edge_clusters):
    """Generate a new graph based on the partitions of a given graph"""
    H = G.__class__()
    node2com = {}
    new_edge_clusters = defaultdict(set) #######
    ...
    for edge in G.edges(data=True):
          com1 = node2com[edge[0]]
          com2 = node2com[edge[1]]
          if com1 == com2:
              new_edge_clusters[(com1, com2)].update(edge_clusters[edge])
    ...
    edge_clusters.update(new_edge_clusters)
    for (com1, com2), edges in new_edge_clusters.items():
        H.add_edge(com1, com2, cluster_edges=edges)
